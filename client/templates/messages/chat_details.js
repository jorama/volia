    ...
    var cancelPopup = $ionicPopup.confirm({
      ...
      buttons: [{
        ...
        onTap: function() {
          ...
          self.rateUser = self.otherUser;
          self.rateChat = self.chat;
  ...
  this.rate = function() {
    ...
    Meteor.call('sendFeedback', thisFeedback, function(err) {
      if (!err) {
        ...
        Meteor.call('insertFeed', 'postFeedback', self.rateUser.profID, null, self.rateUser.profID);
        $scope.modal.remove();
        if (Meteor.isCordova) {
